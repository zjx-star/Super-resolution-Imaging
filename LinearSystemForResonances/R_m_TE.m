function vec = R_m_TE(m,N,l,k,k1,R1,R2,h1,h2,mode)
%Compute R_m^TE
%
%Input:
...
if(m==0)
    ...
    for n = 1:N
        ...
        beta2=accu_beta_N(m,n,R2,h2);
        lambda1=eigenvalue_N(m,0,R1,h1);
        lambda2=eigenvalue_N(m,n,R2,h2);
        s1=s_mn_N(k1,m,0,R1,h1);
        s2=s_mn_N(k1,m,n,R2,h2);
        ...
        c1=@(x)-a*x;
        c2=@(x)1-a*x;
        I5=integral3(f5,0,1,c1,0,0,pi/2)+integral3(f5,0,1,0,c2,0,pi/2)+integral3(f5,0,1,c1,c2,pi/2,pi);
        vec(n)=-2*cot(s2*l/2+pi/2*mode)/lambda2.^(3/4)/lambda1./C1./C2.*...
            ...
            k.^2.*beta1.*(1i.*m1).*integral3(f4,0,1,c1,c2,0,pi)+k.^2.*m1.^2.*I5);
    end
end
end
