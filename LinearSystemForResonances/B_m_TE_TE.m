function mat = B_m_TE_TE(m,N,l,k,k1,R1,R2,h1,h2,mode)
%Construct B_m_TE_TE
%
%Input:
...
if(m==0)
    for(i=0:N*N-1)
        n1=floor(i/N)+1;
        n2=mod(i,N)+1;
        beta1=accu_beta_N(0,n1,R1,h1);
        beta2=accu_beta_N(0,n2,R2,h2);
        s1=s_mn_N(k1,0,n1,R1,h1);
        s2=s_mn_N(k1,0,n2,R2,h2);
        lambda1=eigenvalue_N(0,n1,R1,h1);
        lambda2=eigenvalue_N(0,n2,R2,h2);
        ...
        mat(i+1)=s1^(-1)*-2*cot(s2*l/2+pi/2*mode)/lambda2^(3/4)/lambda1^(1/4)*...
            2*pi*R1.*h1.*R2.*h2.*2*((-lambda1).*lambda2.*I1+k.^2.*beta1.*beta2.*I2)./...
            C1./C2;
    end
else
    m1=m;
    ...
    for(i=0:N*N-1)
        ...
        c1=@(x)-a*x;
        c2=@(x)1-a*x;
        mat(i+1)=s1^(-1)*-2*cot(s2*l/2+pi/2*mode)/lambda2^(3/4)/lambda1^(1/4)./C1./C2.*...
            2*pi*R1*h1*R2*h2.*2*((-lambda1).*lambda2.*integral3(f1,0,1,c1,c2,0,pi)+k.^2.*beta1.*beta2.*integral3(f2,0,1,c1,c2,0,pi)-k.^2.*(-1i.*m1).*beta1.*integral3(f3,0,1,c1,c2,0,pi)+...
            k.^2.*(1i.*m1).*beta2.*integral3(f4,0,1,c1,c2,0,pi)+k.^2.*(m1.^2).*integral3(f5,0,1,c1,c2,0,pi));
    end
end
end
