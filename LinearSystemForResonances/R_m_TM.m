function vec = R_m_TM(m,J,l,k,k1,R1,R2,h1,h2,mode)
%Compute R_m^TM
%
%Input:
...
if(m==0)
    for j=1:J
        ...
        f=@(s1,s2,theta)exp(1i.*k.*sqrt((R2-R1+R2.*h2.*s2).^2+4.*R1.*R2.*(1+h1.*s1).*(1+h2.*(s2+a.*s1)).*sin(theta/2).^2))./(4.*pi.*sqrt((R2-R1+R2.*h2.*s2).^2+4.*R1.*R2.*(1+h1.*s1).*(1+h2.*(s2+a.*s1)).*sin(theta/2).^2)).*...
            (bessely(m,R1.*beta1).*(besselj(m-1,beta1.*(1+(s2+a*s1).*h1))-besselj(m+1,beta1.*R1.*(1+(s2+a*s1).*h1)))./2-besselj(m,R1*beta1).*(bessely(m-1,beta1.*R1.*(1+(s2+a*s1).*h1))-bessely(m+1,beta1.*R1.*(1+(s2+a*s1).*h1)))./2).*...
            cos(theta).*R1.*(1+(a*s1+s2).*h1);        
        %f=@(s1,s2,theta)1./(4.*pi.*sqrt(h.^2.*s2.^2+4.*(1+s1.*h).*(1+(s2+s1).*h).*sin(theta./2).^2)).*...
        %    (bessely(m,beta).*(besselj(m-1,beta.*(1+(s2+s1).*h))-besselj(m+1,beta.*(1+(s2+s1).*h)))./2-besselj(m,beta).*(bessely(m-1,beta.*(1+(s2+s1).*h))-bessely(m+1,beta.*(1+(s2+s1).*h)))./2).*...
        %    cos(theta).*(1+(s1+s2).*h);              
        ...
        c1=@(x)-a*x;
        c2=@(x)1-a*x;
        vec(j)=-cot(s1*l/2+pi/2*mode)./lambda1.^(1/4)./pi./k./log(1+h).*...
            2*pi*k^2*R1*h1*R2*h2.*2.*beta1.*integral3(f,0,1,c1,c2,0,pi)./...
            C;        
    end
else 
    m1=m;
    ...
    for j = 1:J
        ...
        beta2=accu_beta_D(m,j,R2,h2);
        lambda1=eigenvalue_N(m,0,R1,h1);
        lambda2=eigenvalue_D(m,j,R2,h2);
        s1=s_mn_N(k1,m,0,R1,h1);
        s2=s_ij_D(k1,m,j,R2,h2);
        ...
        c1=@(x)-a*x;
        c2=@(x)1-a*x;
        vec(j)=-2*cot(s2*l/2+pi/2*mode)/lambda2.^(1/4)/lambda1./C1./C2.*...
            2.*pi.*R1*h1*R2*h2.*2.*(k.^2.*beta1.*beta2.*integral3(f1,0,1,c1,c2,0,pi)+k.^2.*beta2.*(-1i.*m1).*integral3(f2,0,1,c1,c2,0,pi)+...
            -k.^2.*beta1.*(1i.*m1).*integral3(f3,0,1,c1,c2,0,pi)+k.^2.*m1.^2.*integral3(f4,0,1,c1,c2,0,pi));
    end
end
end
