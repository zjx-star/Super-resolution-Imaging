function val = E_TM_3(m,n,R,h,l,k,r,theta,mode)
...
if(mod(mode,2)==0)
    ...
    val=2.*lambda.^(3/4)./s.*norm^(-1).*(bessely(m,R.*beta).*besselj(m,beta.*r)-besselj(m,R.*beta).*bessely(m,beta.*r)).*exp(1i.*m1.*theta);
else
    beta=accu_beta_D(m,n,R,h);
    s=s_ij_D(k,m,n,R,h);
    lambda=eigenvalue_D(m,n,R,h);
    norm=norm_psi_D(m,n,R,h);
    val=-2*1i*cot(s*l/2).*lambda.^(3/4)./s.*norm^(-1).*(beta.*(bessely(m,R.*beta).*((besselj(m-1,beta.*r)-besselj(m+1,beta.*r))/2)-...
